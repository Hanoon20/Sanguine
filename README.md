...
paused timeline, labelled `seal`, `sealRelease`, `topFlap`, `camera`,
`sideFlaps`, `bottomFlap`, `light`, `photo`, `text`, so any phase can be
scrubbed while tuning.

The camera starts moving at 0.75s, while the flaps are still turning, and does
not stop. That overlap is the whole point: the opening and the move inward
read as one gesture instead of two steps.
