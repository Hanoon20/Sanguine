 ...
 * The photograph opens out of the point the camera is heading for, holds
 * sharp for a beat, then falls out of focus as the wording arrives line by
 * line over it. About six and a half seconds, thumb to reading.
 *
...
export function createEnvelopeOpeningTimeline(
  ...
  addFlapOpen(tl, flap(el.flapTop, FLAP_ANGLES.top, 'rotateX', -7, 2), 0.55, 1.0);

  /* The resting shadow is let go as soon as the envelope starts to open. It
     lies in exactly the space the bottom flap swings down into, and left up it
     covers that flap almost completely. */
  tl.to(el.shadow, { opacity: 0.16, duration: 1.1, ease: 'sine.out' }, 0.55);

  /* The dolly starts here, under the flaps, and runs to the end. */
  tl.addLabel('camera', 0.75);
  addCameraPush(tl, el, 0.75);
  ...
  /* Given a longer swing than the rest. It is the last thing to move and the
     one the eye follows out of the envelope. */
  addFlapOpen(tl, flap(el.flapBottom, FLAP_ANGLES.bottom, 'rotateX', 6, 0.8), 1.42, 1.05);

  tl.addLabel('light', 1.5);
  addGoldenLight(tl, el, 1.5);

  tl.addLabel('photo', 2.55);
  addPhotoReveal(tl, el, 2.55);
  tl.call(() => onCardVisible?.(), undefined, 4.3);

  tl.addLabel('text', 4.55);
  addTextReveal(tl, el, 4.55);
