...
export function addSealActivation(tl: gsap.core.Timeline, e: RevealElements, at: number) {
  tl.to([e.hint, e.overline], { opacity: 0, y: 8, duration: 0.3, ease: 'power2.out' }, at)
    .to(e.sealBody, { scale: 1.075, rotate: -2.2, duration: 0.24, ease: EASE.wax }, at)
    .to(e.sealGlow, { opacity: 0.85, scale: 1.5, duration: 0.36, ease: 'power2.out' }, at)
    .to(e.sealBody, { '--wax-lit': 1, duration: 0.28, ease: 'sine.out' }, at);
  return tl;
}
...
export function addSealRelease(tl: gsap.core.Timeline, e: RevealElements, at: number) {
  ...
  tl.to(e.sealBody, { scale: 0.86, opacity: 0, duration: 0.34, ease: 'power2.in' }, at + 0.05)
    .to(e.sealGlow, { opacity: 0, scale: 2.1, duration: 0.42, ease: 'power2.out' }, at + 0.08)
    .set(e.seal, { pointerEvents: 'none' }, at + 0.4)
...
export function addPhotoReveal(tl: gsap.core.Timeline, e: RevealElements, at: number) {
  tl.set(e.backdrop, { willChange: 'transform, filter' }, at)
    ...
    .to(e.message, { opacity: 0, y: -8, duration: 0.7, ease: 'power2.in' }, at - 0.1)
